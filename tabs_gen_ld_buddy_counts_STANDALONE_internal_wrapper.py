...
def wait_for_processes(processes):
	...
	*OBS* THIS FUNCTION IS TO BE USED IN SCRIPTS WITH A LOGGER!
	...
	"""
	logger.info( "FUNCTION wait_for_processes() CALLED!" )
	...
	for param in processes.keys():
		...
		logger.info( "[pid={pid}; param={param}; job_no={job_no}; log_file={log_file}]".format(pid=pid, param=param, job_no=job_no, log_file=log_file) )
	...
	for param in processes.keys():
		...
		logger.info( "WATING <-- [pid={pid}; param={param}; job_no={job_no}; log_file={log_file}]".format(pid=pid, param=param, job_no=job_no, log_file=log_file) )
		
		p.wait()
		# elapsed_time = time.time() - start_time
		# logger.info( "DONE in %s s (%s min)" % (elapsed_time, elapsed_time/60) )
	...
	for param in processes.keys():
		...
		logger.info( "RETURN CODE={returncode} <-- [pid={pid}; param={param}; job_no={job_no}; log_file={log_file}]".format(returncode=p.returncode, pid=pid, param=param, job_no=job_no, log_file=log_file) )
		...
		for distance_type in distance_types:
			...
			for param in param_list:
				...
				if (job_no % 5 == 0):
					...
					logger.info( "resetting processes by re-initialyzing the defauldict" )
					processes = collections.defaultdict(dict) # resetting processes

	logger.info( "Waiting for last round of processes" )
	wait_for_processes(processes)
...
logger = pplogger.Logger(name=log_name, log_dir=log_dir, log_format=0, enabled=True).get() #

...
class LaunchBsub(object):
	LB_job_counter = 0
	LB_job_fails = 0
	def __init__(self, cmd, queue_name, walltime, mem, jobname='NoJobName', logdir=os.getcwd(), log_root='unknown_root_name', file_output= __name__+'.tmp.out', no_output=False, email=False): #file_output=os.path.join(os.getcwd(), __name__+'.tmp.log'
        ...
        if email:
        	...
        	self.bcmd = "bjobs -J {jobname} -o {output} -r -q {queue} -W {walltime} -R 'rusage[mem={mem}]' -N -u {email}".format(jobname=self.jobname,  output=self.file_output, queue=self.p_queue_name, walltime=self.p_walltime, mem=self.p_mem, email=self.email) 
        else:
        	self.bcmd = "bjobs -J {jobname} -o {output} -r -q {queue} -W {walltime} -R 'rusage[mem={mem}]'".format(jobname=self.jobname,  output=self.file_output, queue=self.p_queue_name, walltime=self.p_walltime, mem=self.p_mem) 

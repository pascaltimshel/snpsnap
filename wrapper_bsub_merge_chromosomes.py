...
def submit():
	...
	for super_population in param_super_population:
		...
		if gen_test_data:
			input_merge_file="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_test_duplicate_rm/{super_population}/plink_merge_file.txt".format(super_population=super_population)
		else:
			input_merge_file="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_full_duplicate_rm{super_population}/plink_merge_file.txt".format(super_population=super_population)
		with open(input_merge_file, 'w') as f:
			for chromosome in param_chromosome: # **LOOPING OVER CHROMOSOME**#
				if gen_test_data:
					line2write="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_test_duplicate_rm/{super_population}/ALL.chr{chromosome}.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes".format(super_population=super_population, chromosome=chromosome) # OBS: this is the prefix needed in PLINK
				else:
					line2write="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_full_duplicate_rm/{super_population}/ALL.chr{chromosome}.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes".format(super_population=super_population, chromosome=chromosome) # OBS: this is the prefix needed in PLINK
				f.write(line2write+"\n")
		...
		if gen_test_data:
			dir_out="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_test_duplicate_rm_merged/{super_population}".format(super_population=super_population)
		else:
			dir_out="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_full_duplicate_rm_merged/{super_population}".format(super_population=super_population)
		...
		cmd_plink = "plink --merge-list {input_merge_file} --make-bed --out {out_prefix}".format(input_merge_file=input_merge_file, file_duplicates=file_duplicates, out_prefix=out_prefix)
		logger.info( "Making plink call:\n%s" % cmd_plink )

		jobname = None
		if gen_test_data:
			jobname = "merge_test_" + super_population # e.g merge_EUR
		else:
			jobname = "merge_full_" + super_population # e.g merge_EUR
...
###################################### SETUP logging ######################################
current_script_name = os.path.basename(__file__).replace('.py','')
log_dir = "/cvar/jhlab/snpsnap/logs_pipeline/production_v2/step1_merge_chromosomes" #OBS

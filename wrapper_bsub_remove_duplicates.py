...
def submit():
	...
	for super_population in param_super_population:
		...
		for chromosome in param_chromosome:
			...
			input_bed = None
			if gen_test_data:
				input_bed="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_test/{super_population}/ALL.chr{chromosome}.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes".format(super_population=super_population, chromosome=chromosome) # OBS: this is the prefix needed in PLINK
			else:
				input_bed="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_full/{super_population}/ALL.chr{chromosome}.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes".format(super_population=super_population, chromosome=chromosome) # OBS: this is the prefix needed in PLINK
			...
			if gen_test_data:
				dir_out="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_test_duplicate_rm/{super_population}".format(super_population=super_population)
			else:
				dir_out="/cvar/jhlab/snpsnap/data/step1/production_v2_QC_full_duplicate_rm/{super_population}".format(super_population=super_population)
			...
			cmd_plink = "plink --bfile {input_bed} --exclude {file_duplicates} --make-bed --out {out_prefix} --noweb".format(input_bed=input_bed, file_duplicates=file_duplicates, out_prefix=out_prefix)
			logger.info( "Making call '--make-bed and QC':\n%s" % cmd_plink )

			jobname = None
			if gen_test_data:
				jobname = "rm_dup_test_" + super_population + "_chr_" + str(chromosome) # e.g rm_dup_EUR_chr_21
			else:
				jobname = "rm_dup_full_" + super_population + "_chr_" + str(chromosome) # e.g rm_dup_EUR_chr_21
